...
// Lightweight test fixture for attach_memory_hook — no communicator, no
// rank/size, no store. The CCA singleton starts uninitialized (nullptr).
class AttachMemoryHookTest : public ::testing::Test {
 protected:
  void SetUp() override {
...
TEST_F(AttachMemoryHookTest, IsIdempotent) {
  auto hook = std::make_unique<CachingAllocatorHookMock>();
  hook->setupDefaultBehaviors();
  CachingAllocatorHook::setInstance(std::move(hook));

  EXPECT_NO_THROW(TorchCommFactory::get().attach_memory_hook("ncclx"));
  EXPECT_NO_THROW(TorchCommFactory::get().attach_memory_hook("ncclx"));
}

TEST_F(AttachMemoryHookTest, RegDeregWorksAfterAttach) {
